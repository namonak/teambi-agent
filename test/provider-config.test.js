...
test('모델 이름의 공백·CR을 정리하고 오염 사실을 알린다', async () => {
  const c = await geminiConfig({ GEMINI_MODEL: 'gemini-2.5-flash\r' }, 'dirtymodel');
  assert.equal(c.model, 'gemini-2.5-flash', '정리 후 정상 인식되어야 한다');
  assert.ok(
...
test('providerStatus가 프로바이더 설정을 함께 싣는다', async () => {
  ...
  assert.equal(s.config.model, 'gemini-2.5-flash');
});
