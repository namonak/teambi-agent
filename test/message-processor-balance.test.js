// message-processor-balance.test.js — 회신에 붙는 잔액 줄 생성.
...
before(async () => {
  ...
  processor = await import('../src/message-processor.js');
});
...
test('조회 성공: 카테고리 이름과 잔액/예산을 한 줄로 만든다', async () => {
  mode = 'ok';
  const line = await processor.balanceLineFor('2026-08', 7);
  assert.match(line, /^\n커피 잔액: /);
...
test('해당 카테고리가 없으면 빈 문자열 (회신에 아무것도 붙지 않음)', async () => {
  mode = 'ok';
  const line = await processor.balanceLineFor('2026-08', 999);
  assert.equal(line, '');
});
